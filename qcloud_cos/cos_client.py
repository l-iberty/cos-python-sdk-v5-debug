...
class CosS3Client(object):
    ...
    def send_request(self, method, url, bucket, timeout=30, cos_request=True, ci_request=False, **kwargs):
        ...
        for j in range(self._retry + 1):
            logger.debug('send request. url:%s, retry_time:%d, method:%s, pid:%d' % (url, j, method, os.getpid()))
            try:
                if j != 0:
                    time.sleep(j)
                if method == 'POST':
                ...
                if res.status_code < 400:  # 2xx和3xx都认为是成功的
                    logger.debug("send request done#1. url:%s, method:%s, pid:%d" % (url, method, os.getpid()))
                    return res
        ...
        logger.debug("send request done#2. url:%s, method:%s, pid:%d" % (url, method, os.getpid()))
        return None
